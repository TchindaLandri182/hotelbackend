...
exports.createRegion = [
  async (req, res) => {
    try {
      ...
      res.status(201).json({ 
        messageCode: 'MSG_0037', message: 'Region created successfully',
        region: newRegion
      });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (!region || region.deleted) {
        return res.status(404).json({ messageCode: 'MSG_0038', message: 'Region not found' });
      }
      ...
      res.json({ 
        messageCode: 'MSG_0039', message: 'Region updated successfully',
        region
      });
    } catch (error) {
    ...
    try {
      ...
      if (citiesCount > 0) {
        return res.status(400).json({ 
          messageCode: 'MSG_0040', message: 'Cannot delete region with associated cities' 
        });
      }
      ...
      res.json({ 
        messageCode: 'MSG_0041', message: 'Region deleted successfully'
      });
    } catch (error) {
