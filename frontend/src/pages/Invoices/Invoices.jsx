...
const Invoices = () => {
  ...
  const getHotelInfo = (invoice) => {
    // Mock hotel info - in real app, get from invoice.stay.room.hotel
    return {
      name: invoice.stay.room.hotel?.name || 'Hotel Name',
      address: invoice.stay.room.hotel?.address || 'Hotel Address',
      phone: '+237 675619114',
      email: invoice.stay.room.hotel?.owners[0]?.email || 'Email'
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      renderCell: (params) => `${params.row.totalAmount} FCFA`,
    },
    {
      field: 'issueDate',
      ...
      renderCell: (params) => (
        <Box>
          <PDFDownloadLink
            ...
            fileName={`facture-${params.row._id.slice(-6).toUpperCase()}.pdf`}
          >
