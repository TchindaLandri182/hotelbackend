...
const InvoiceForm = () => {
  ...
  const handleSubmit = async (values) => {
    const invoiceData = {
      stay: values.stay,
      totalAmount: values.payments.reduce((acc, current) => acc + current.amountPaid, 0) || parseFloat(values.totalAmount) ,
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} md={6}>
                      <Autocomplete
                        ...
                        getOptionLabel={(option) => {
                          ...
                          return `${option.client?.firstName} ${option.client?.lastName} - ${option.room?.roomNumber}/${option?.room.category?.basePrice} FCFA  (${start.toLocaleDateString()} - ${end.toLocaleDateString()}, ${diffDays} days)`;
                        }}
                        value={stays.find(stay => stay._id === values.stay) || null}MM
                        onChange={(_, newValue) => {
                          ...
                          const diffTime = end.getTime() - start.getTime();
                          const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                          setToPay(diffDays * newValue?.room.category?.basePrice)

                          setFieldValue('stay', newValue?._id || '')}
                        }
                        renderInput={(params) => (
                          <TextField
                            {...params}
                    ...
                    <Grid item xs={12} md={6}>
                      <Field
                        ...
                        type="number"
                        disabled={true}
                        value={values.payments.reduce((acc, current) => acc + current.amountPaid, 0)}
                        fullWidth
                        variant="outlined"
                        InputProps={{
                          readOnly: true,
                          startAdornment: <Typography sx={{ mr: 1, width: 220 }}>{toPay ? `NAP ${toPay} FCFA  |` : ''}</Typography>,
